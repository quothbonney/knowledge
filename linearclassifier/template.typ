...
#let proof(body) = {
  block(
    width: 100%,
    fill: rgb("#F8F9FA"),  // Very light gray, almost white
    radius: 2pt,           // Subtle rounded corners
    inset: (x: 12pt, y: 8pt),
    stroke: (
      paint: rgb("#E0E0E0"),  // Light gray border
      thickness: 0.5pt
    )
  )[
    #text( weight: "bold")[#smallcaps("Proof"):]
    #h(4pt)
    #body
    #v(4pt)
    #align(right)[
      #text[*$square.stroked$ Q.E.D.*]  // Darker gray end symbol
    ]
  ]
}
